...
const TELEMETRY_ENV_DISABLE: &str = "LOCAL_SSL_TELEMETRY_DISABLE";
...
impl Telemetry {
    pub fn load(data_dir: &PathBuf) -> Self {
        ...
        if let Ok(json) = fs::read_to_string(&config_path) {
            ...
            Telemetry { uuid, enabled, first_seen: now, last_ping: now, usage_days: 0, config_path }
        }
    }
    ...
    pub fn maybe_heartbeat(&self) -> bool {
        ...
        let payload = format!(
            "uuid={}&tool=local-ssl&version={}&os={}&event=heartbeat&age_days={}&ts={}",
    ...
    pub fn status(&self) -> String {
        let s = if self.enabled { "enabled ✓".green() } else { "disabled".yellow() };
        let days = if self.usage_days == 0 { "first day!".green() } else { format!("{} days", self.usage_days).cyan() };
        format!(
            "Anonymous telemetry: {s}\n\
             UUID:                   {}\n\
             Using since:            {}\n\
             Tool age:               {days}\n\
             Last heartbeat:         {}\n\
             \n\
             To change:\n  local-ssl telemetry enable\n  local-ssl telemetry disable\n  local-ssl telemetry status\n\
             \n\
             Publish URL: {PUBLISH_URL}",
            self.uuid, format_ts(self.first_seen), format_ts(self.last_ping),
        )
    }
    ...
    pub fn show_heartbeat_notice(&self) {
        if self.enabled && !TELEMETRY_URL.is_empty() {
            ...
            println!("{}", "ℹ Telemetry: set LOCAL_SSL_TELEMETRY_URL to enable anonymous usage reporting.".dimmed());
        }
    }
